...
target_url = target_url + "/Reports_"  + '_' + str(anno) + '_' + str(mes) + '_' + str(dia) + '_' + str(hora) + '_' + str(minuto) + '_' + str(segundo) 
folder = (
    ctx.web.default_document_library()
    .root_folder.folders.add_using_path(target_url, overwrite=True)
    .execute_query()
)
target_folder = ctx.web.get_folder_by_server_relative_url(target_url)
