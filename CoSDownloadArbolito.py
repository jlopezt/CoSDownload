...
for fichero in ficheros:
    ...
    payload={'entityType':'reports', 'id': fichero["url"]}
    ...
    if(x.status_code==200): 
        ...
        form_fields=response['jsonConfiguration']['form_fields']
        field_value={}
        fields={}
        filters={}

        for form_field in form_fields:
            if form_field['field_type']=="json" and form_field['field_system_name']=='configuration': #Es la lista de campos
                fields_value=form_field['field_value']
                fields=fields_value['fields']
                filters=fields_value['filters']
                entityType=fields_value['entityType']
        ...
        if fields!={}:
            ...
            if(x.status_code==200): 
                ...
                #report
                print("Iniciando report/csv")
                url= url_csv#url_report
                ...
                if(x.status_code==200): 
                    ...
                    if response['size']>0:
                        ...
                        """
                        #CSV
                        nombreFichero=response["originalName"]#el nombre que guardo
                        nombre=response["name"] #el nombre que pido
                        ...
                        if(x.status_code==200): 
                            ...
                            print(f"Solicitud de descarga fallida. Codigo {x.status_code}")
                else:
                    print(f"Solicitud de report/csv fallida. Codigo {x.status_code}")
            else:
                print(f"Solicitud de sync fallida. Codigo {x.status_code}")
        else:
            print(f"Fields vacio en inicio de solicitud.")
    else:
        print(f"Inicio de solicitud fallida. Codigo {x.status_code}")